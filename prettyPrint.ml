...
let add_string_limit sl s wid force =
...
let int_get_patterns ppenv exp mode prec =
    let same_functor e1 e2 = match (e1,e2) with
          | ((APPL (f1,l1)),(APPL (f2,l2))) -> f1=f2
          | (_,_) -> false in
    let non_appl e = match e with
          | (APPL (f,l)) -> false
          | _ -> true in
...
and recPpProcessBreak ppenv exp wid one_line sl prefix markers indent (ParsePattern (s,prec,e,p)) (ol,bl) parse_marks force =
    ...
    let update_holder sl pos (p,sx,sy,ex,ey) (il,s,e) =
    ...
    let rec process sl holders markers n l1 l2 l3 = match (l1,l2,l3) with
          ...
          | ((tf::tr),(olf::olr),(blf::blr)) ->
            let new_holders = update_holders sl n holders parse_marks in
            let add_tok sl marksers t force = match t with
            ...
            let add_nl_tok sl marksers indent t force = match t with
                       | (ID s) -> (add_string_limit sl s wid force,markers)
                       | (SYMBOL s) -> (add_string_limit sl s wid force,markers)
                       | (NUMBER n) -> (add_string_limit sl (string_of_int n) wid force,markers)
                       | (SPECIAL " ") -> (sl,markers)
...
let ppExp ppenv exp wid =
    let _ = p_cache := Disc.newExpItem in
    let (f,r) = ppenv in
...
let build_trie patterns mode =
    ...
    let similar_token t1 t2 = match (t1,t2) with
    ...
    let rename_token n1 n2 t = match t with
          | (TVAR (s1,s2,prec)) ->
            if s1 = n1 then
                (TVAR (n2,s2,prec))
            else
...
let int_mk_failure l = match l with
  | ((t,f,r,c,_,_)::_) -> (f,r,c)
  | [] -> ("*END",0,0) ;;
...
and trie_parse trie mode prec l =
    ...
    in
        ...
        let _ = if trl=[] then raise (ParseFailure (mk_failure l)) in
        let continue_rule tl prec rest sub indl =
         ...
         let continue_parse exp indices exp_prec ret_prec rest sub =
                let _ = if trl=[] then raise (ParseFailure (mk_failure rest)) in
...
and trie_parse_no_prefix trie mode prec l =
    ...
    let _ = if l=[] then raise (ParseFailure (mk_failure l)) in
        (*val tok = hd l*)
    let rest = l in
