...
let add_and_derive rewrite env l = match l with
  | [x] ->
...
let add_or_derive rewrite env l = match l with
  | [x] ->
...
let build_env rewrite env e residue = match e with
  | (APPL (s,l)) ->
    let env1 =
            if residue=[] then
                env
                    ...
                    List.map (fun (x,l,u) ->
                        ...
                        (List.fold_left List.append [] (List.map
                         (fun (x,ll) ->
                          ...
                          let _c1 = ExpIntern.intern_exp (Renv.isACorC env) c in
