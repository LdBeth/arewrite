...
let addSmall d el =
    ...
    let _ = List.iter (fun (x) -> Array.set add (hashLeft (ExpIntern.decode_exp (REF x))) ((REF x)::(Array.get add (hashLeft (ExpIntern.decode_exp (REF x)))))) el in
        add ;;
...
let rec add ac d e = match e with
  | (APPL (v,[l;r;c])) ->
    let _ = ExpIntern.intern_exp ac (APPL (v,[l;r;c])) in
