...
let returns_bool env e = match e with
  | (QUANT (14,_,_,_)) -> true
  | (QUANT (15,_,_,_)) -> true
  | (APPL (s,l)) ->
    (*print_string "Here1\n";*)
...
let make_subst vars e = match e with
...
let rat_plus (RATIONAL (a,b)) (RATIONAL (c,d)) =
    let n = a*d+b*c in
    let d = b*d in
    let g = gcd n d in
...
let mkRatBrTerm l = match l with
  | [] -> (RATIONAL (1,1))
  | [x] -> x
  | l -> (APPL (78,l)) ;;
...
let getTermType env e = match e with
...
let find_inversion env e = match e with
  | (CASE (e,_,cl)) ->
...
let invert_clauses f (CASE (e,t,cl)) =
...
let reduce_finite_case ee t cases = match ee with
  | (APPL (f,[])) ->
    let rec find_case l = match l with
          | [] -> []
          | ((c,e)::r) ->
...
let builtin rewrite env e = match e with
