...
let elaborate_phase env oper terms =
    ...
    let combine_terms one_ref two_ref =
    ...
    let combine_rev_terms one_ref two_ref =
            PairsMap.fold (fun (f,s) -> (fun nl -> (fun (st,dt) ->
    ...
    let new_transitive_terms sd dd terms =
    ...
    let rec transitive_closure sd dd terms =
            let
                ...
                represented_by e f = match (e,f) with
                  | (_,(APPL (5,[]))) -> true
                  | ((APPL (5,[])),_) -> false
                  | (op1,op2) ->
                    (same_operands oper op1 op2) &&
...
let rewrite_op (t,c,e) env cond_terms terms =
